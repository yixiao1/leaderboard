...
class FramesStacking_agent(object):
    ...
    def setup(self, path_to_conf_file):
        ...
        if g_conf.MODEL_TYPE in ['FramesStacking_SpeedLoss']:
    ...
    def run_step(self, inputs_data):
        ...
        actions_outputs, attention_layers, _ = self._model.forward_eval(norm_rgb, direction)
        ...
        if self.attention_save_path:
            ...
            #input.paste(command_sign, (180, 10), command_sign)
            ...
            draw.text((35, 150), str("Steer " + "%.3f" % steer), fill=(0, 0, 255), font=font)
            draw.text((225, 150), str("Throttle " + "%.3f" % throttle), fill=(0, 0, 255), font=font)
            draw.text((400, 150), str("Brake " + "%.3f" % brake), fill=(0, 0, 255), font=font)
            #draw.text((670, 230), str("Speed " + "%.3f" % speed), fill=(0, 0, 255), font=font)

            blend_im.save(os.path.join(self.attention_save_path, str(self.att_count).zfill(6) + '.png'))
