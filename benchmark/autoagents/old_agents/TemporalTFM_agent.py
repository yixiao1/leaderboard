...
class TemporalTFM_agent(object):
    ...
    def run_step(self, inputs_data):
        ...
        actions_outputs, att_backbone_layers, attn_weights = self._model.forward_eval(norm_rgb, direction)
    ...
    def saving_backbone_attention_maps(self, last_input, att_backbone_layers, inputs_data=None, action=None):
    ...
    def saving_TxEncoder_attention_maps(self, cat_inputs, attn_weights):
        ...
        att_mat = torch.stack(attn_weights).squeeze(1)  # (num_layers, S+1, T+1)
        ...
        mask = v[0, 1:].reshape(1, v.shape[-1]-1).detach().numpy()   # (1, T)
