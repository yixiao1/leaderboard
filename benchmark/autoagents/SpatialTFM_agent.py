...
class SpatialTFM_agent(object):
    ...
    def setup(self, path_to_conf_file):
        ...
        if g_conf.MODEL_TYPE in ['SpatialTFM_SpeedLoss']:
    ...
    def run_step(self, inputs_data):
        ...
        src_patches, (self.x_num, self.y_num) = self.create_image_patches(norm_rgb, g_conf.IMAGE_SHAPE, g_conf.MODEL_CONFIGURATION['patch_size'])
        direction = [torch.cuda.FloatTensor(self.process_command(inputs_data[int(i/(self.x_num*self.y_num))]['GPS'][1], inputs_data[int(i/(self.x_num*self.y_num))]['IMU'][1])[0]).unsqueeze(0)
                     for i in range(len(src_patches))]
        actions_outputs, patches_layers, attn_weights = self._model.forward_eval(src_patches, direction)
        steer, throttle, brake = self.process_control_outputs(actions_outputs.detach().cpu().numpy().squeeze(0))
        ...
        if self.attention_save_path:
            ...
            #self.saving_backbone_attention_maps(last_input, len(norm_rgb), src_patches, patches_layers,
            #                                    inputs_data =inputs_data, action=[steer, throttle, brake])
            self.saving_TxEncoder_attention_maps(last_input, len(norm_rgb), len(src_patches), attn_weights)
    ...
    def create_image_patches(self, imgs, img_shape, patch_size, overlap=False):
        """
        Divide a single-image into some patches and return a list.
        Args:
            imgs: a list of the source images. [(B, C, H, W)]
            patch_size: the patch size you want to cut (h, w)
        ...
        if not overlap:
            ...
            return patch_list, (x_num, y_num)
        else:
            # TODO
            pass

    # we saving the attention maps of the perception backbone
    def saving_backbone_attention_maps(self, last_input, inputs_num, src_patches, patches_layers, inputs_data=None, action=None):
        if not os.path.exists(self.attention_save_path+'_backbone_attn'):
            os.makedirs(self.attention_save_path+'_backbone_attn')

        # take out the last frame's patches attention maps
        patches_att_list = []
        for i in range(-int(len(src_patches) / inputs_num), 0):
            att = np.delete(self.cmap_2(np.abs(patches_layers[i][-1][0].cpu().data.numpy()).mean(0) / np.abs(
                patches_layers[i][-1][0].cpu().data.numpy()).mean(0).max()), 3, 2)
            patches_att_list.append(att)

        # combine patches to be a single image
        x_att = []
        for x in range(0, len(patches_att_list), self.x_num):
            x_att.append(np.concatenate(patches_att_list[x:x + self.x_num], 0))
        att = np.concatenate(x_att, 1)
        ...
        if inputs_data is not None:
            ...
            if action is not None:
                ...
                draw.text((35, 150), str("Steer " + "%.3f" % action[0]), fill=(0, 0, 255), font=font)
                draw.text((225, 150), str("Throttle " + "%.3f" % action[1]), fill=(0, 0, 255), font=font)
                draw.text((400, 150), str("Brake " + "%.3f" % action[2]), fill=(0, 0, 255), font=font)
                # draw.text((670, 230), str("Speed " + "%.3f" % speed), fill=(0, 0, 255), font=font)

        blend_im.save(os.path.join(self.attention_save_path+'_backbone_attn', str(self.att_count).zfill(6) + '.png'))

    # we saving the attention maps of the last multi-head attention layer of the transformer encoder
    def saving_TxEncoder_attention_maps(self, last_input, inputs_num, patches_num, attn_weights):
        if not os.path.exists(self.attention_save_path+'_TxEncoder_attn'):
            os.makedirs(self.attention_save_path+'_TxEncoder_attn')

        att_mat = torch.stack(attn_weights).squeeze(1)  # (num_layers, S, T)
        ...
        mask = v[0, -int(patches_num / inputs_num):].reshape(self.y_num, self.x_num).detach().numpy()
        mask = cv2.resize(mask / mask.max(), last_input.size)
        mask = Image.fromarray((np.delete(self.cmap_2(mask), 3, 2) * 255).astype('uint8'))
        attention_map = Image.blend(last_input, mask, 0.7)
        attention_map.save(
